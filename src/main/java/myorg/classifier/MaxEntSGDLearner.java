...
public class MaxEntSGDLearner {
    ...
    public static void learnWithStochasticLoop(
       ...
       float eta0, float lambda, int numIters, WeightMatrix wMatrix, WeightVector bVector
    ) {
        ...
        for (int i = 1; i <= numIters; i++) {
            ...
            learnWithStochasticOneStep(fVec, eta, lambda, wMatrix, bVector);
        }
    }

    public static void learnWithStochasticOneStep(
        FeatureVector fVec, float eta, float lambda, WeightMatrix wMatrix, WeightVector bVector
    ) {
        int y = (int)fVec.getLabel();
        if (y < 0 || wMatrix.getRowDimensions() < y || bVector.getDimensions() < y) {
            return;
        }
        ...
        for (int i = 0; i < expArray.length; i++) {
            float e = (float)Math.exp(expArray[i] + bVector.getValue(i));
            expArray[i] = e;
            expSum += e;
        }

        wMatrix.scale(1.0f - eta * lambda);
        bVector.scale(1.0f - eta * lambda);
        ...
        for (int i = 0; i < expArray.length; i++) {
            ...
            float b = bVector.getValue(i) + t * eta;
            bVector.setValue(i, b);
        }
    }
}
